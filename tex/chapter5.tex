...
We thoroughly understand that the range of the data is not sufficient to perform an accurate differential study (as the error values clearly show). We nevertheless believe that we deeply understood the nature of the problem. We are sure that in the future we will be able to set up runs capable of reaching bulk-Richardson numbers of one order of magnitude higher than the ones obtained for this work. This will be achieved primarily by using a non-uniform Cartesian grid which is implemented in SHL This will help us better refine the convective boundary regions without the same precision on the stable layers above and below.
