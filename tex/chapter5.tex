...
\begin{figure}[t!]
  ...
  \centering
  \hfill
  \subfloat[The same passive scalar of above, advected at $t \simeq 800 \ \mathrm{min}$. This defines the topology of the boundary. In the magnification a region of fully convective fluid with density of passive scalar close to unity (see text).]{\includegraphics[width=12cm]{./img/passive2acc2.pdf}\label{fig:passiveacc2}}
...
The entrainment plotted over time in Figure (\ref{fig:3dpsprofile}) is also roughly linear and qualitatively resembles the 2D result, with the upper and loer boundaries that are migrating upward and downward respectively.
\begin{figure}[t!]
      \centering
