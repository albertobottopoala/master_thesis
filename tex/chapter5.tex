...
\begin{figure}[t!]
  ...
  \subfloat[Dependency of the Mach number on the heating rate (see text).]{\includegraphics[width=0.5\textwidth]{./img/differentialmach.pdf}\label{fig:diffrms}}
      \hfill
        \subfloat[Dependency of the buoyancy jump on the initial Brunt-Väisälä frequency.]{\includegraphics[width=0.5\textwidth]{./img/differentialdelb.pdf}\label{fig:diffjump}}
	\hfill
...
\begin{figure}[t!]
	...
	\label{fig:3dsingle}
\end{figure}
	After having correctly tuned the heating function in a 2D setup, we switched to a 3D setup. Only one simulation was run: 3d0.10-100, where the $0.1$ stands again for the fraction $\nabla T / \nabla$ and $100$ stands for the percentage of the heating referring to the arbitrary value of $1.8 \times 10^{18} \mathrm{erg/s}$. As in the 2D case, the heating function has a Gaussian profile peaked in the convective zone and decreasing in time.

	The physical setup was mapped on a uniform Cartesian grid with $512 \times 256 \times 512$ cells. The simulation was run on the JUQUEEN machine at JSC and scaled over an entire rack ($16384$ cores) for four days. Although SLH scales very well even on tens of thousands of cores, on JUQUEEN a very high number of CPU hours was needed, primarily because the PowerPC\textregistered \ A2 processors (IBM) of JUQUEEN are optimized to obtain the highest efficiency in terms of computations per watt, rather that per core. 

	Only one grid file every 500 time steps was saved, hence the lower resolution in time for the plots compared to the 2D analysis. This was not necessary to reduce the output size (around $300 \, \mathrm{GB}$ total), but to avoid to spend too many CPU hours in I/O processing.


\subsection{Evolution of a 3D run}

In the following paragraphs we analyze the run 3d0.1-100.
