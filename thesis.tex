...
In our case for the 2D runs it will be a box of $2.4 \times 10^{9} cm$ on the $x$ axis and $1.2 \times 10^{9} cm$ on the $y$ axis. Gravity is constant pointing downward on the $y$ axis with a magnitude of $1000 cm/s^2$. This generates a pressure statification in the fluid that covers approximately three pressure scale heigh. \\
The controlling parameter is the temperature gradient that can we initialized to any value. In out case we divided the simulated region in three parts as shown in \ref{fig:tempprofile}. We define a parameter $alpha$ which is nothing but the fraction of the $\nabla$ over the $\nabla_{\mathrm{ad}}$. As seen in previous sections $alpha<1$ implies stability, the layer is instable otherwise. \\
We always initialized the lower and upper region to be stable and the central region to be unstable. Convection will arise hence in the center and over time expand upward and downward. The advantage of simulating a strip of convection between two stable layers (miming a shell convection) instead of a convective region at the bottom that grows upward (Core convection-like) is twofold: it gives us two convective boundaries to study, and we avoid high mach number at the boundary which can at times generate problems or unphysical situations. \\
Furthermore the convection is stimulated by a heating function with a gaussian profile centered in the original convective region (see \label{fig:tempprofile}).
\begin{figure}[t]
...
\centering
\end{figure}
A wide range of boundary condition has been tested for this problem. Periodic boundary conditions along the $y$-axis would have provided the most physical solution, unfortunately a shear flow used to appear at the onset of convection. We obtained the same shear flow using boundary with constant extrapolation for the velocity field.
