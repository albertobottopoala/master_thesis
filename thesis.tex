...
Photons instead, interacting electromagnetically, scatter multiple times before reaching the \textit{photosphere}, the region where statistically speaking they scatter for the last time before ending up in our eyes or our telescopes.

Whithout investigeting any thoroughly the nature of energy generation, we simply note
...
\end{equation}
Let's define three new quantities
...
\end{equation}
Since pressure is always decreasing at increasing radius we can measure the temperaure as a function of the pressure, and so its gradient. \\
With this new notation we can write our stability criterion in a more compact way as
\begin{equation}\label{stabcritcomp}
	\nabla < \nabla_e + \frac{\phi}{\delta} \nabla_{\mu}
\end{equation}
In our simulation we used a monoatocim ideal gas equation of state, hence $\nabla_{\mu}=0$. We shall now obtain an analytic expression for $\nabla$.
